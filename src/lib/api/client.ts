...
export async function fetchCorridor(): Promise<CorridorResult> {
...
export async function fetchCaseFileMetadata(): Promise<CaseFileMetadataResult> {
  const res = await fetch(`${API_BASE}/api/case-file/metadata`);
  if (!res.ok) throw new Error(`Failed to fetch case file metadata: ${res.statusText}`);
  return res.json();
}

export function getCaseFilePdfUrl(): string {
  return `${API_BASE}/api/case-file`;
}

export async function fetchAisTracks(): Promise<AisTracksResult> {
