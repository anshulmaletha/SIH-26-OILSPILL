...
def get_detection(scenario: str = Query("active", description="Scenario: active | rejected | rejected_lookalike")):
    ...
    if not SAR_DETECTION_FILE.exists():
        raise HTTPException(status_code=404, detail="sar_detection_output.json not found")

    with open(SAR_DETECTION_FILE, "r", encoding="utf-8") as f:
...
def get_corridor():
    ...
    """
    if not H3_CORRIDOR_FILE.exists():
        raise HTTPException(status_code=404, detail="h3_corridor_output.json not found")

    with open(H3_CORRIDOR_FILE, "r", encoding="utf-8") as f:
        data = json.load(f)
...
def get_case_file_metadata():
    ...
    if not CASE_FILE_JSON.exists():
        raise HTTPException(status_code=404, detail="case_file_output.json not found")
